...
loop={true}
playsInline={true}
className={`w-element ${"bg-video"}`}>
<source
type={"video/mp4"}
src={LandingPagesData?.data?.data?.carolinaLp?.[0]?.customHtml}
className={`w-element`} />
</video>
}
<div
...
{(LandingPagesData?.data?.data?.carolinaLp?.[0]?.subhead ? true : false) &&
<p
className={`w-element c18v83wh ce6x08i c1lvj0n c1kqoumg`}>
{LandingPagesData?.data?.data?.carolinaLp?.[0]?.subhead}
</p>
}
<h1
className={`w-element c1r3nuqs c1ihu3w4 c1lvj0n c1tdj1xa c1yzjm0o ce6x08i c4c2xkh`}>
{LandingPagesData?.data?.data?.carolinaLp?.[0]?.header}
</h1>
</div>
<p
...
{(LandingPagesData?.data?.data?.carolinaLp?.[0]?.images3?.length ? true : false) &&
<div
className={`w-element`}>
<div
className={`w-element c4vc9qx c9tjkc5 c5zgd1 c11b3qd4 c7iyr7r cmpwmbp`}>
...
{Object.entries(
  // @ts-ignore
  LandingPagesData?.data?.data?.carolinaLp?.[0]?.images3 ?? {}
).map(([_key, collectionItem]: any) => {
  const index = Array.isArray(LandingPagesData?.data?.data?.carolinaLp?.[0]?.images3) ? Number(_key) : _key;
  return (
<Fragment key={index}>
<Image
...
{LandingPagesData?.data?.data?.carolinaLp?.[0]?.footerAddress}
</p>
</div>
</div>
<div
