...
let TeamCollectionData = useResource("TeamCollectionData_1")
return <Body
className={`w-element`}>
<Box
...
{Object.entries(
  // @ts-ignore
  TeamCollectionData?.data?.data?.ourTeams ?? {}
).map(([_key, collectionItem]: any) => {
  const collectionItemKey = Array.isArray(TeamCollectionData?.data?.data?.ourTeams) ? Number(_key) : _key;
  return (
...
{collectionItem?.jobTitle}
</p>
{(collectionItem?.bio ? true : false) &&
<p
className={`w-element c309ipl`}>
{collectionItem?.bio}
</p>
